            ...
            <div className='admin_clients' style={{margin: "200px auto"}}>
                <div className="card" style={{width: "26rem"}}>
                    <div className="d-flex flex-column align-items-center py-3 px-2">
                        <h5 className="card-title fs-4 fw-bold">Клиент: {client.name}</h5>
                    </div>
                    <ul className="list-group list-group-flush">
                        <li className="list-group-item"><b>Телефон:</b> {client.phone}</li>
                        <li className="list-group-item"><b>Город:</b> {client.city}</li>
                        <li className="list-group-item"><b>Авто:</b> {client.car}</li>
                        <li className="list-group-item"><b>Заказы:</b>
                            {
                                client.orders && client.orders.map(order =>
                                    <p key={order._id}>
                                        {
                                            typeof order.order_date === "string" ? order.order_date.slice(0, 10) : order.order
                                        } -
                                        {order.diagnosis ? "На диагностике" : null}
                                        {order.in_progress ? "На обслуживании" : null}
                                        {order.is_finished ? "Обслуживание завершено" : null}
                                    </p>
                                )
                            }
                        </li>
                    ...
                    <div className="d-flex justify-content-evenly py-3 px-2">
                        <button type="button" className="btn btn-warning" id="editClientBtn"
                                onClick={() => setModal(true)}>Изменить
                        </button>
                        <button type="button" className="btn btn-danger" id="deleteClientBtn"
                                onClick={DeleteClient}>Удалить
                        </button>
                    </div>
                </div>
            </div>
